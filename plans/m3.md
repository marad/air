...
**Acceptance Criteria**:
- [x] `{{include "path"}}` syntax correctly includes file contents
- [x] Relative paths resolved from current file's directory
- [x] Absolute paths work correctly
- [x] Nested includes work (includes can contain includes)
- [x] Circular includes detected and rejected with clear error
- [x] Missing included files produce clear error messages
- [x] Include processing happens before frontmatter parsing
...
**Acceptance Criteria**:
- [x] `{{variable}}` syntax replaced with variable value
- [x] `{{variable|default}}` uses default when variable undefined
- [x] Variables from frontmatter resolved correctly
- [x] Missing variables without defaults produce clear error
- [x] Variable names support letters, numbers, underscores (must start with letter/underscore)
- [x] Placeholder replacement happens after include processing
- [x] Manual testing with all test templates succeeds
...
**Acceptance Criteria**:
- [x] `--var key=value` CLI flag parsing works
- [x] Multiple `--var` flags supported
- [x] Variables from YAML frontmatter loaded into Config
- [x] Environment variables accessible in templates
- [x] Precedence order enforced: CLI > frontmatter > env
- [x] Invalid `--var` format produces clear error
- [x] Usage message updated to show `--var` option
- [x] Manual testing with all scenarios succeeds
...
- [x] Task 9: File inclusion implemented and tested
- [x] Task 10: Placeholder replacement implemented and tested
- [x] Task 11: Variable resolution from CLI/env/frontmatter working
- [x] Circular include detection working
- [x] All test templates created and tested
- [x] Error handling for all edge cases implemented
- [x] Example templates with reusable components created
- [x] README updated with templating documentation
...
- [x] Backward compatibility maintained (old templates still work)
- [x] Ready to use for complex multi-file prompt engineering
