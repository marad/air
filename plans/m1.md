...
**Acceptance Criteria**:
- [x] CLI accepts filename as first argument
- [x] File contents are read into memory
- [x] Clear error message for missing files
- [x] Usage message displayed when no arguments
- [x] Exit code 1 on errors, 0 on success
...
**Acceptance Criteria**:
- [x] Successfully extracts YAML between `---` delimiters
- [x] Returns markdown content without frontmatter
- [x] Handles files without frontmatter (treats entire file as content)
- [x] Clear error for malformed YAML
- [x] Clear error for unclosed frontmatter
...
**Acceptance Criteria**:
- [x] `.env` file loaded on startup
- [x] Environment variables set correctly
- [x] Graceful handling when `.env` doesn't exist
- [x] Warning (not error) for malformed `.env`
- [x] Works even if env vars already set system-wide
...
**Acceptance Criteria**:
- [x] Markdown content (without frontmatter) sent to Vertex AI
- [x] API errors handled gracefully with clear messages
- [x] Exit code 1 on API errors
- [x] Error messages provide context (which step failed)
...
**Acceptance Criteria**:
- [x] AI response printed to stdout
- [x] Clean output (no extra decorations)
- [x] Proper newline handling
- [x] Works well with shell redirection: `./air template.md > output.txt`
